...
z = [];
while length(y)~=2000   % zbieramy 1000 pomiarow
...
fprintf('Hp: %d, Hs: %d\n', Hp, Hs);
czas = input('Jaki by³ czas (CH1)? ','s');
title(sprintf('Regulacja GPC: Tp = %.2fs, Lambda = 15, Hp = %d, Hs = %d (czas iteracji = %s)', Tp, Hp, Hs, czas));
ylim([-2000,2000]);
name = ['C:\Users\Admin\Documents\GitHub\LaTeX\CZASOPISMO2_Artyku³\obrazki\',sprintf('tp50ms_lambda15_Hp%d_Hs%d', Hp, Hs)];
savefig(name);
saveas(1, name, 'png');
f = fopen([name,'.txt'],'w');
fprintf(f,'k, t, u, y, z\n');
